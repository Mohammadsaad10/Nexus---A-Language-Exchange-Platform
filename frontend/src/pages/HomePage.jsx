...
const HomePage = () => {
  ...
  const { mutate: sendRequestMutation , isLoading: sendRequestLoading } = useMutation({
    ...
    onError: (error) => {
      if (error.response?.data?.type === "INCOMING_REQUEST_EXISTS") {
        toast.error("This user has already sent you a friend request!");
        // Navigate to notifications page so they can accept the request
